...
class ScryfallImport
{
    ...
    public static function scryfallImport(
        ...
        $insertSql = sprintf("INSERT INTO
                                ...
                                api_uri, scryfall_uri, layout, image_uri, illustration_id, manacost,
                                ...
                                f1_flavor, f1_image_uri, f1_illustration_id, f1_cmc, f1_printed_name, f1_flavor_name,
                                f2_name, f2_manacost, f2_power, f2_toughness, f2_loyalty,
                                f2_type, f2_ability, f2_colour, f2_artist, f2_flavor,
                                f2_image_uri, f2_illustration_id, f2_cmc, f2_printed_name, f2_flavor_name, p1_id,
                            ...
                            ON DUPLICATE KEY UPDATE
                                ...
                                f1_illustration_id = IF(
                                    NOT (content_hash <=> VALUES(content_hash)),
                                    VALUES(f1_illustration_id),
                                    f1_illustration_id
                                ),
                                ...
                                f2_illustration_id = IF(
                                    NOT (content_hash <=> VALUES(content_hash)),
                                    VALUES(f2_illustration_id),
                                    f2_illustration_id
                                ),
        ...
        $bind = $stmt->bind_param(
            ...
            . "sssssssssssssssssssssssssssssssssssssssssssssssssssssssssii",
            $id,
            $oracle_id,
            $tcgplayer_id,
            $multi_1,
        ...
        try {
            foreach ($data as $key => $value) :
                ...
                $illustration_id = $value["illustration_id"] ?? null;
                ...
                elseif ($skip === 0) :
                    ...
                    if (!empty($cardFaces)) :
                        ...
                        foreach ($cardFaces as $value3) :
                            ...
                            if (isset($value3["illustration_id"])) :
                                ${'illustration_id_' . $face_loop} = $value3["illustration_id"];
                            endif;
                            if (isset($value3["cmc"])) :
    ...
    public static function importTags(
        ...
        try {
            foreach ($tagImports as $importType) :
                ...
                $url = static::requireGameRuleUrl($gameRules, $config['rule']);
                $msg->logMessage('[NOTICE]', "Scryfall {$config['label']} API: fetching $url");
                $bulkInfo = static::fetchJson($url, $msg, "Scryfall {$config['label']} API", $appConfig);
                ...
                $downloadResult = static::getBulkDataFile($downloadUri, $config['file'], $maxFileAge, $appConfig);
                if ($downloadResult === false) :
                    throw new \Exception("[ERROR] scryfall_tag_definitions: {$config['label']} data download failed");
                endif;

                $summary[] = static::importTagFile(
                    $config,
                    $importType,
                    $db,
                    $msg,
    ...
    */
    private static function importTagFile(
        ...
        try {
            foreach (static::iterateBulkRecords($config['file']) as $value) :
                $countTags++;
                $grandTotalTags++;
