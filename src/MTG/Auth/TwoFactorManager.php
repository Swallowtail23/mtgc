...
class TwoFactorManager
{
    ...
    */
    ...
    public function enable($user_id, $method = 'email')
    {
        ...
        if ($method === 'app') :
            ...
            if ($success) :
                ...
                $this->directLog(
                    '[NOTICE]',
                    "2FA (app) provisioning URI stored in session ({$_SESSION['tfa_provisioning_uri']})"
                );
    ...
    public function disable($user_id)
    {
        ...
        $query = "UPDATE users SET tfa_enabled = 0, tfa_method = NULL, tfa_backup_codes = NULL, 
            tfa_app_secret = NULL WHERE usernumber = ?";
        $stmt = $this->db->prepare($query);
        $stmt->bind_param("i", $user_id);
        $success = $stmt->execute();
    ...
    public function startVerification($user_id, $email)
    {
        ...
        elseif ($method === 'app') :
            $this->directLog(
                '[NOTICE]',
                "App-based 2FA selected for user ID: $user_id. Waiting for code verification."
            );
