...
if (empty($_GET)) :
        ...
        if (strlen($name) > 2 || !empty($setcoderegexsearch)) : // Needs >2 chars to search
            ...
            elseif ($allprintings === "yes") :  // Used in 'All Printings' search from card_detail page
                ...
                $criteria = "(cards_scry.name LIKE ? OR cards_scry.f1_name LIKE ? OR cards_scry.f2_name LIKE ?
                            OR cards_scry.printed_name LIKE ? OR cards_scry.f1_printed_name LIKE ?
                            OR cards_scry.f2_printed_name LIKE ? OR cards_scry.flavor_name LIKE ?
                            OR cards_scry.f1_flavor_name LIKE ? OR cards_scry.f2_flavor_name LIKE ?) ";
    ...
    elseif ($adv == "yes") :
        ...
        elseif (
            ...
            and empty($searchtype)
            and empty($searchsetcode)
        ...
        elseif (!empty($searchpromo) and !empty($name)) :
            ...
            if ($searchname === "yes") :
                if ($exact === "yes") :
                    $criteriaNTA = "(
                        cards_scry.name = ? OR cards_scry.f1_name = ? OR cards_scry.f2_name = ?
                        OR cards_scry.printed_name = ? OR cards_scry.f1_printed_name = ?
                        OR cards_scry.f2_printed_name = ? OR cards_scry.flavor_name = ?
                        OR cards_scry.f1_flavor_name = ? OR cards_scry.f2_flavor_name = ?) ";
                    $params = array_fill(0, 9, $name);
                else :
                    $criteriaNTA = "(
                        cards_scry.name LIKE ? OR cards_scry.f1_name LIKE ? OR cards_scry.f2_name LIKE ?
                        OR cards_scry.printed_name LIKE ? OR cards_scry.f1_printed_name LIKE ?
                        OR cards_scry.f2_printed_name LIKE ? OR cards_scry.flavor_name LIKE ?
                        OR cards_scry.f1_flavor_name LIKE ? OR cards_scry.f2_flavor_name LIKE ?) ";
                    $params = array_fill(0, 9, "%{$name}%");
        ...
        if ($white === "yes") :
            $criteriaCol = "(
                cards_scry.color LIKE '%W%' OR cards_scry.color_identity LIKE '%W%'
                OR cards_scry.f1_colour LIKE '%W%' OR cards_scry.f2_colour LIKE '%W%' )";
        endif;
        if ($blue === "yes") :
            ...
            $criteriaCol .= "(
                cards_scry.color LIKE '%U%' OR cards_scry.color_identity LIKE '%U%'
                OR cards_scry.f1_colour LIKE '%U%' OR cards_scry.f2_colour LIKE '%U%' )";
        endif;
        if ($black === "yes") :
            if (!empty($criteriaCol)) :
                $criteriaCol .= $colourOp . " ";
            ...
            $criteriaCol .= "(
                cards_scry.color LIKE '%B%' OR cards_scry.color_identity LIKE '%B%'
                OR cards_scry.f1_colour LIKE '%B%' OR cards_scry.f2_colour LIKE '%B%' )";
        endif;
        if ($red === "yes") :
            if (!empty($criteriaCol)) :
                $criteriaCol .= $colourOp . " ";
            ...
            $criteriaCol .= "(
                cards_scry.color LIKE '%R%' OR cards_scry.color_identity LIKE '%R%'
                OR cards_scry.f1_colour LIKE '%R%' OR cards_scry.f2_colour LIKE '%R%' )";
        endif;
        if ($green === "yes") :
            if (!empty($criteriaCol)) :
                $criteriaCol .= $colourOp . " ";
            ...
            $criteriaCol .= "(
                cards_scry.color LIKE '%G%' OR cards_scry.color_identity LIKE '%G%'
                OR cards_scry.f1_colour LIKE '%G%' OR cards_scry.f2_colour LIKE '%G%' )";
        endif;
        if ($colourless === "yes") :
            if (!empty($criteriaCol)) :
                $criteriaCol .= $colourOp . " ";
            ...
            $criteriaCol .= "(
                cards_scry.color LIKE '%[]%' OR cards_scry.f1_colour LIKE '%[]%'
                OR cards_scry.f2_colour LIKE '%[]%' )";
        endif;
        if (!empty($criteriaCol)) :
        ...
        if ($colourExcl == "ONLY") :
            if (empty($white)) :
                $criteria .= "
                    and ((cards_scry.color IS NULL OR cards_scry.color NOT LIKE '%W%')
                    and (cards_scry.color_identity IS NULL OR cards_scry.color_identity NOT LIKE '%W%')
                    and (cards_scry.f1_colour IS NULL OR cards_scry.f1_colour NOT LIKE '%W%')
                    and (cards_scry.f2_colour IS NULL OR cards_scry.f2_colour NOT LIKE '%W%')) ";
            endif;
            if (empty($blue)) :
                $criteria .= "
                    and ((cards_scry.color IS NULL OR cards_scry.color NOT LIKE '%U%')
                    and (cards_scry.color_identity IS NULL OR cards_scry.color_identity NOT LIKE '%U%')
                    and (cards_scry.f1_colour IS NULL OR cards_scry.f1_colour NOT LIKE '%U%')
                    and (cards_scry.f2_colour IS NULL OR cards_scry.f2_colour NOT LIKE '%U%')) ";
            endif;
            if (empty($red)) :
                $criteria .= "
                    and ((cards_scry.color IS NULL OR cards_scry.color NOT LIKE '%R%')
                    and (cards_scry.color_identity IS NULL OR cards_scry.color_identity NOT LIKE '%R%')
                    and (cards_scry.f1_colour IS NULL OR cards_scry.f1_colour NOT LIKE '%R%')
                    and (cards_scry.f2_colour IS NULL OR cards_scry.f2_colour NOT LIKE '%R%')) ";
            endif;
            if (empty($green)) :
                $criteria .= "
                    and ((cards_scry.color IS NULL OR cards_scry.color NOT LIKE '%G%')
                    and (cards_scry.color_identity IS NULL OR cards_scry.color_identity NOT LIKE '%G%')
                    and (cards_scry.f1_colour IS NULL OR cards_scry.f1_colour NOT LIKE '%G%')
                    and (cards_scry.f2_colour IS NULL OR cards_scry.f2_colour NOT LIKE '%G%')) ";
            endif;
            if (empty($black)) :
                $criteria .= "
                    and ((cards_scry.color IS NULL OR cards_scry.color NOT LIKE '%B%')
                    and (cards_scry.color_identity IS NULL OR cards_scry.color_identity NOT LIKE '%B%')
                    and (cards_scry.f1_colour IS NULL OR cards_scry.f1_colour NOT LIKE '%B%')
                    and (cards_scry.f2_colour IS NULL OR cards_scry.f2_colour NOT LIKE '%B%')) ";
            endif;
        endif;
        // New
        $criteriaNew = "";
        ...
        elseif ($scope === "mycollection" && !empty($collqtyoperator)) :
            if ($collqtyoperator === "ltn") :
                $criteria .= "
                    AND (
                        (COALESCE($mytable.normal, 0) + COALESCE($mytable.foil, 0) + COALESCE($mytable.etched, 0)) < ?
                    )
                    AND (
                        (COALESCE($mytable.normal, 0) + COALESCE($mytable.foil, 0) + COALESCE($mytable.etched, 0)) > 0
                    ) ";
        ...
        if (!empty($sortBy)) :
            if ($sortBy == "auto") :
                ...
                $isEmptyCollectionSearch = (
                    $scope === "mycollection"
                    and $searchname === "yes"
                    and empty($collqtyoperator)
                    and (isset($name) and empty($name))
                );

                if ($plstName || $plstRegex || $plstSelected) :
                    $order = "
                        ORDER BY set_date DESC,
                        (SELECT sets.release_date
                            FROM sets
                            WHERE sets.code =
                                SUBSTRING(cards_scry.number_import, 1, LOCATE('-', cards_scry.number_import) - 1)
                        )
                        DESC,
                        ...
                        COALESCE(cards_scry.flavor_name, cards_scry.name) ASC,
                        cs_id ASC ";
                elseif ($sldName || $sldRegex || $sldSelected) :
                    $order = "
                        ORDER BY
                            set_date DESC,
                            cards_scry.release_date DESC,
                            cards_scry.set_name ASC,
                            primary_card DESC,
                            cards_scry.number ASC,
                            CAST(REGEXP_REPLACE(number_import, '[[:alpha:]]', '') AS UNSIGNED) ASC,
                            number_import ASC,
                            COALESCE(cards_scry.flavor_name, cards_scry.name) ASC,
                            cs_id ASC
                ...
                elseif ($new === "yes") :
                    $order = "
                        ORDER BY
                            ...
                            cs_id ASC
                    ";
                elseif ($isEmptyCollectionSearch) :
                    $order = "
                        ORDER BY
                            $mytable.topvalue DESC,
                            COALESCE(cards_scry.flavor_name, cards_scry.name) ASC,
                            set_date DESC,
                            primary_card DESC,
                            number ASC,
            ...
            elseif ($sortBy == "name") :
                $order = "ORDER BY COALESCE(cards_scry.flavor_name, cards_scry.name) ASC,
                    set_date DESC, primary_card DESC, number ASC, cs_id ASC ";
            elseif ($sortBy == "price" and $scope === "mycollection") :
                $order = "ORDER BY $mytable.topvalue DESC,
                    COALESCE(cards_scry.flavor_name, cards_scry.name),
                    set_date DESC, primary_card DESC, number ASC, cs_id ASC ";
            elseif ($sortBy == "price") :
                $order = "ORDER BY cards_scry.price_sort DESC,
                    COALESCE(cards_scry.flavor_name, cards_scry.name) ASC,
                    set_date DESC, primary_card DESC, number ASC, number_import ASC, cs_id ASC ";
            elseif ($sortBy == "cmc") :
            ...
            elseif ($sortBy == "cmcdown") :
                $order = "ORDER BY cards_scry.cmc DESC,
                    ...
                    set_date DESC, primary_card DESC, number ASC, cs_id ASC ";
            elseif ($sortBy == "set") :
                $order = "ORDER BY set_date ASC, cards_scry.set_name ASC,
                    primary_card DESC, cards_scry.number ASC,
                    COALESCE(cards_scry.flavor_name, cards_scry.name) ASC ";

            // DEFAULT
            elseif ($sortBy == "setdown") :  // Set down number up
                $order = "ORDER BY set_date DESC, cards_scry.set_name ASC, primary_card DESC, number ASC,
                    cards_scry.release_date ASC,
                    CAST(REGEXP_REPLACE(number_import, '[[:alpha:]]', '') AS UNSIGNED) ASC,
                    number_import ASC, COALESCE(cards_scry.flavor_name, cards_scry.name) ASC, cs_id ASC ";
            elseif ($sortBy == "setnumberdown") : // Set down number down
                $order = "ORDER BY set_date DESC, cards_scry.set_name ASC, primary_card DESC, number DESC,
                    COALESCE(cards_scry.flavor_name, cards_scry.name) ASC,
                    CAST(REGEXP_REPLACE(number_import, '[[:alpha:]]', '') AS UNSIGNED) ASC,
