...
// Pass data to this form by e.g. ?setabbrv=BNG&number=1&id=123456 
...
if (isset($_GET["setabbrv"])):
    $setabbrv = filter_input(INPUT_GET, 'setabbrv', FILTER_SANITIZE_STRING); 
elseif (isset($_POST["setabbrv"])):
    $setabbrv = filter_input(INPUT_POST, 'setabbrv', FILTER_SANITIZE_STRING);     
endif;
if (isset($_GET["id"])):
...
<div id="page">
    ...
    // Check that we have a number and a setabbrv before calling SQL query
    if (isset($_GET["number"]) OR isset($_POST["number"])) :
        $number = $db->escape($number,'int');
        if (isset($_GET["setabbrv"]) OR isset($_POST["setabbrv"])) :
            $setabbrv = $db->escape($setabbrv,'str');
            if (isset($_GET["id"]) OR isset($_POST["id"])) :
                ...
                if (!$qtyresults == 0) :
                    ...
                    $flipcard = $db->escape(fliptype($row['backid'],$row['manacost'],$row['meld']),'str');
                    $obj = new Message;$obj->MessageTxt('[NOTICE]',basename(__FILE__)." ".__LINE__,"Flipcard type is $flipcard",$logfile);
                    //Populate JSON data
                    $scryfalljson = scryfall($setcode,$id,null,$cardname,$cardnumber);
                    if(isset($scryfalljson['id'])):
                        $scryfallid = $db->escape($scryfalljson['id'],'str');
                        if(isset($scryfalljson['layout']) AND $scryfalljson['layout'] === "transform" AND $flipcard === "front"):
                            $scryfallimg = $scryfalljson['card_faces']['0']['image_uris']['normal'];
                        elseif(isset($scryfalljson['layout']) AND $scryfalljson['layout'] === "transform" AND $flipcard === "back"):
                            $scryfallimg = $scryfalljson['card_faces']['1']['image_uris']['normal'];
                        elseif(isset($scryfalljson['layout']) AND $scryfalljson['layout'] === "normal"):
                            $scryfallimg = $scryfalljson['image_uris']['normal'];
                        elseif(isset($scryfalljson['layout']) AND $scryfalljson['layout'] === "flip"):
                            $scryfallimg = $scryfalljson['image_uris']['normal'];
                        elseif(isset($scryfalljson['layout']) AND $scryfalljson['layout'] === "meld"):
                            $scryfallimg = $scryfalljson['image_uris']['normal'];
                        else:
                            $scryfallimg = null;
                        endif;
                    ...
                    $obj = new Message;$obj->MessageTxt('[DEBUG]',basename(__FILE__)." ".__LINE__,"Call for getimgname by $useremail with $setcode, $number, $cardid, $flipcard",$logfile);
                    $imgname = getimgname($setcode, $number, $cardid, $flipcard);
                    $obj = new Message;
                    $obj->MessageTxt('[DEBUG]',basename(__FILE__)." ".__LINE__,"Call for getImageNew by $useremail with $setcode,$imgname,$row[0],$ImgLocation, $flipcard, $cardnumber, $cardname, $scryfallimg",$logfile);
                    $imageurl = getImageNew($setcode,$imgname,$row[0],$ImgLocation,$flipcard,$cardnumber,$cardname,$scryfallimg);
                    if (($flipcard != 'no') AND ($flipcard != 'bfzfull')): 
                        ...
                        if($flipresult = $db->query($getflip)):
                            $obj = new Message;$obj->MessageTxt('[DEBUG]',basename(__FILE__)." ".__LINE__,"Flip SQL query succeeded",$logfile);
                        else:
                            trigger_error("[ERROR]".basename(__FILE__)." ".__LINE__.": SQL failure: " . $db->error, E_USER_ERROR);  
                        endif;
                        
                        // Hacked out for now with a row count and a temp variable ($noflipresult)
                        
                        if($flipresult->num_rows > 0):
                            $fliprow = $flipresult->fetch_array(MYSQLI_BOTH); //$fliprow now contains all card info about the flip side
                            $reverseflipcard = $db->escape(fliptype($fliprow['backid'],$fliprow['manacost'],$fliprow['meld']),'str');
                            $flipcardname = stripslashes($db->escape($fliprow['name'],'str'));
                            $flipcardnumber = $db->escape($fliprow['number'],'int');
                            $obj = new Message;
                            $obj->MessageTxt('[DEBUG]',basename(__FILE__)." ".__LINE__,"Call for getflipimgname by $useremail with $flipcard, $setcode, $number, {$fliprow[0]}, {$fliprow['number']}",$logfile);
                            $flipimgname = getflipimagename($flipcard,$setcode,$number,$fliprow[0],$fliprow['number']);
                            if (($flipcard === 'back') OR ($flipcard === 'kamigawaflipbottom')):
                                $id = "$backid"; //So update queries increment FRONT not BACK
                            endif;
                            $obj = new Message;
                            $obj->MessageTxt('[DEBUG]',basename(__FILE__)." ".__LINE__,"Call for getImageNew for flip by $useremail with $setcode,$flipimgname,$fliprow[0],$ImgLocation, $flipcard, $cardnumber, $flipcardname",$logfile);
                    ...
                    if (($flipcard === 'no') OR ($flipcard === 'bfzfull') OR ($flipcard === 'front') OR 
                            ($flipcard === 'kamigawafliptop') OR ($flipcard === 'meldmain') OR ($flipcard === 'meldadd') ): 
                        if (empty($row['normal'])):
                            $myqty = 0;
                        else:
                    ...
                    elseif (($flipcard === 'back') OR ($flipcard === 'kamigawaflipbottom')):
                        if (empty($fliprow['normal'])):
                            ...
                            $myqty = $db->escape($fliprow['normal'],'int');
                        endif;
                        if (empty($fliprow['foil'])):
                            $myfoil = 0;
                        else:
                            $myfoil = $db->escape($fliprow['foil'],'int');
                        endif;
                    endif;
                    ...
                    if (isset($_POST['update'])) :    
                        ...
                        if($sqlcheck = $db->select('*',"`$mytable`","WHERE id = '$sqlid'")):
                    ...
                    ?>
                            ...
                            <table>
                                <tr>
                                    <td class="h2pad">
                                        <?php 
                                        if($noflipresult !== 1):
                                            if (($flipcard !== 'no') AND ($flipcard !== 'bfzfull') AND ($flipcard !== 'meldcombined')):
                                                $fliprow['name'] = htmlentities($fliprow['name'],ENT_QUOTES,"UTF-8");
                                                echo $row['name']." / ".$fliprow['name'];
                                            else:
                                                echo $row['name'];
                                            endif;
                                        endif;
                        ...
                        <div id="minicarddetailheader">
                            ...
                            if($noflipresult !== 1):
                                if (($flipcard !== 'no') AND ($flipcard !== 'bfzfull') AND ($flipcard !== 'meldcombined')):
                                    $fliprow['name'] = htmlentities($fliprow['name'],ENT_QUOTES,"UTF-8");
                                    echo "<h2 class = 'h2pad'>".$row['name']." / ".$fliprow['name']."</h2>";
                                else:
                        ...
                        <div id="carddetailmain">
                            <div id="carddetailimage">
                                <table>
                                    <tr> 
                                        <td colspan="2">
                                            <?php 
                                            if(substr($flipcard,0,8) === 'kamigawa'):
                                                $lookupid = substr($row[0],0,-2);
                                            else:
                                                $lookupid = $row[0];
                                            endif;
                                            $lookupid = htmlentities($lookupid,ENT_QUOTES,"UTF-8");
                                            ...
                                            echo "<a href='http://gatherer.wizards.com/Pages/Card/Details.aspx?multiverseid=".$lookupid."' target='_blank'><img alt='$lookupid' id='cardimg' src=$imagelocation></a>"; ?>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td colspan="1" class="previousbutton">
                                            <?php if ($row['number'] > 1):
                                                ...
                                                $prevcard = $number - 1;
                                                if($prevcardresult = $db->select_one('id','cards_scry',"WHERE setcode = '$setabbrv' AND number = $prevcard ORDER BY manacost DESC")):
                                                    $obj = new Message;
                                        ...
                                        <td colspan="1" class="nextbutton">
                                            <?php if (($row['number'] < $settotal) OR (empty($settotal))): 
                                                // Find the next number card's ID
                                                $nextcard = $number + 1;
                                                if($nextcardresult = $db->select_one('id','cards_scry',"WHERE setcode = '$setabbrv' AND number = $nextcard ORDER BY manacost DESC")):
                                                    $obj = new Message;
                                                    $obj->MessageTxt('[DEBUG]',basename(__FILE__)." ".__LINE__,"SQL query succeeded",$logfile);
                            ...
                            <div id="carddetailinfo">
                                ...
                                if(isset($admin) AND $admin == 1):
                                    echo "<br>$setname (".strtoupper($setcode).") no. ".$row['number']." ID:<a href='admin/cards.php?cardtoedit=$lookupid' target='blank'> ".$lookupid."</a><br>"; 
                                else:
                                    echo "<br>$setname (".strtoupper($setcode).") no. ".$row['number']." ID: ".$lookupid."<br>";
                                endif;
                                if (!empty($row['comment'])):
                                    echo "<b>Comment: </b>".$row['comment']."<br><br>";
                                endif;
                                if ($flipcard === 'back'):
                                    echo "Double sided card (back). Front half: ".$row['backid']."<br>";
                                elseif ($flipcard === 'front'):
                                    echo "Double sided card (front). Back half: ".$row['backid']."<br>";
                                elseif(substr($flipcard,0,8) === 'kamigawa'):
                                    $row['backid'] = substr($row['backid'],1);
                                    echo "Kamigawa flip card. Other half: ".$row['backid']."<br>";
                                endif;
                                //Build links 
                        ...
                        if($noflipresult !== 1):
                                ...
                                <div id="carddetailflip">
                                    <div id="carddetailflipimg">    
                                        <table>
                                           <tr> 
                                                <td colspan="2">
                                                    <?php echo "<a href='http://gatherer.wizards.com/Pages/Card/Details.aspx?multiverseid=".$lookupid."' target='_blank'><img alt='$lookupid' id=\"cardimg\" src=$imageurlback></a>"; ?>
                                                </td>
                                            </tr>
                                        </table>
                                        <?php
                                        if ($flipcard == 'meldmain'): 
                                            ?>
                                    ...
                                    <div id="carddetailflipinfo">
                                        ...
                                        <?php 
                                        echo "<b>Type: </b>".$fliprow['type'];
                                        echo "<br>";
                                        echo "<b>Rarity: </b>";
                                        if (strpos($fliprow['rarity'],"R") !== false):
                                            echo "Rare";
                                        elseif (strpos($fliprow['rarity'],"M") !== false):
                                            echo "Mythic Rare";
                                        elseif (strpos($fliprow['rarity'],"U") !== false):
                                            echo "Uncommon";
                                        else:
                                            echo "Common";
                                        endif;
                                        echo "<br>";
                                        $flipmanacost = symbolreplace($fliprow['manacost']);
                                        echo "<b>Mana cost: </b>".$flipmanacost;
                                        echo "<br>";
                                        echo "<b>CMC: </b>".$fliprow['cmc']; 
                                        echo "<br><br>";
                                        $flipability = str_replace('£','<br>',$fliprow['ability']);
                                        $flipability = symbolreplace($flipability);
                                        echo "<b>Abilities: </b>".$flipability;
                                        echo "<br>";
                                        if (strpos($fliprow['type'],'reature') !== false):
                                            echo "<b>Power / Toughness: </b>".$fliprow['power']."/".$fliprow['toughness']; 
                                        elseif (strpos($fliprow['type'],'laneswalker') !== false):
                                            echo "<b>Loyalty: </b>".$fliprow['loyalty']; 
                                        endif;
                                        echo "<br>";
                                        echo "<b>Art by: </b>".$fliprow['artist'];
                                        if(substr($flipcard,0,8) === 'kamigawa'):
                                            $row['backid'] = substr($row['backid'],0,-2);
                                        endif;
                                        if(isset($admin) AND $admin == 1):
                                            echo "<br>".$row['set_name']." (".$row['cs_setcode'].") no. ".$row['number']." ID:<a href='admin/cards.php?cardtoedit=".$row['backid']."' target='blank'> ".$row['backid']."</a><br>";
                                        else:
                                            echo "<br>".$row['set_name']." (".$row['cs_setcode'].") no. ".$row['number']." ID: ".$row['backid']."<br>";
                                        endif;
                                        $flipflavor = symbolreplace($fliprow['flavor']);
                            ...
                            elseif ($flipcard == 'meldcombined'): ?>
                                <div id="meldcardsinfo">
                                    ...
                                    if($runmeld0 = $db->select_one('name,number','cards_scry',"WHERE cards_scry.id = '$meldcards[0]'")):
                                        $obj = new Message;$obj->MessageTxt('[DEBUG]',basename(__FILE__)." ".__LINE__,"SQL query succeeded",$logfile);
                                    ...
                                    if($runmeld1 = $db->select_one('name,number','cards_scry',"WHERE cards_scry.id = '$meldcards[1]'")):
                                        $obj = new Message;$obj->MessageTxt('[DEBUG]',basename(__FILE__)." ".__LINE__,"SQL query succeeded",$logfile);
                            ...
                            $page_url = strtok(get_full_url(),'?')."?setabbrv=".$setcode."&number=".$number."&id=".$cardid;
                ...
                echo '<h3>Error</h3>Set code not supplied';
            endif;
        else:
            echo '<h3>Error</h3>Card collector number not supplied';
        endif; ?>
    </div>
</div>
<?php 
