        ...
        if ($key == 'data') :
            foreach ($value as $key2 => $value2) :
                ...
                if (isset($value2["metadata"])) :
                    ...
                    $metadata_id = $metadata_lang = $metadata_name = $metadata_set_code = $metadata_oracle_id
                        = $metadata_collector_number = null;
                endif;
                ...
                if ($stmt === false) :
                    ...
                    //In db, but safety check failed
                        ...
                        $msg->logMessage(
                            '[DEBUG]',
                            "$old_scryfall_id exists in existing data and safety check failed, "
                            . "$need_action to be actioned"
                        );
                        $action_text = $action_text
                            . "Old ID: $myURL/carddetail.php?id=$old_scryfall_id\n"
                    ...
                    elseif ($stmt->num_rows > 0 && $deleteCheck > 0) :
                        ...
                        $msg->logMessage(
                            '[NOTICE]',
                            "$old_scryfall_id exists in existing data but not safe to delete, "
                            . "$need_action to be actioned"
                        );
                        $action_text = $action_text
                            . "Old ID: $myURL/carddetail.php?id=$old_scryfall_id\n"
                ...
                $stmt = $db->prepare(
                    "INSERT INTO
                        `migrations`
                            (id, performed_at, object, migration_strategy, uri, old_scryfall_id, new_scryfall_id, note,
                            metadata_id, metadata_lang, metadata_name, metadata_set_code, metadata_oracle_id,
                            metadata_collector_number, db_match)
                        VALUES
                            (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
                );
                if ($stmt === false) :
