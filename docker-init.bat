...
REM Normalize slashes and append mtgc
...
REM Copy placeholder config if not already present
...
if not exist "%BASE_DIR%\config\php_custom.ini" (
    ...
    copy "setup\php_custom.ini" "%BASE_DIR%\config\php_custom.ini"
)
...
REM Check if db-data volume already exists
set DO_DB_SETUP=1
...
REM Wait for MySQL to become available
...
REM Run bulk import if not already done
if not exist "%MARKER_FILE%" (
