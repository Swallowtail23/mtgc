...
class ImportExport
{
    ...
    public function exportCollectionToCsv($table, $myURL, $smtpParameters, $format = 'echo', $filename = 'export.csv', $username = '', $useremail = '')
    {
        ...
        $sql = "SELECT setcode,number_import,name,normal,lang,$table.foil,$table.etched,$table.id as scryfall_id FROM $table JOIN cards_scry ON $table.id = cards_scry.id WHERE (($table.normal > 0) OR ($table.foil > 0) OR ($table.etched > 0))";
    ...
    public function importCollection($filename, $mytable, $importType, $useremail, $serveremail, $importFormat = 'mtgc') {
        ...
        $total = 0;
        $warningsummary = '';
        $warningheading = 'Warning type, Row number, Setcode, Number, Import Name, Import Normal, Import Foil, Import Etched, Supplied ID, Database Name (if applicable), Database ID (if applicable)';
        ...
        while (($data = fgetcsv ($handle, 100000, $delimiter)) !== FALSE):
            ...
            elseif(isset($data[0]) AND isset($data[1]) AND isset($data[2])):  // We have bare minimum info - a setcode, a number and a name
                ...
                if (!empty($data0) AND !empty($data1) AND !empty($data2) AND $idimport === 0): 
